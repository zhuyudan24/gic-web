...
import hyzxtext from '../../../static/img/hyzxtext.png'
...
export default {
  ...
  methods: {
      ...
      if(this.saveBtnBool){
        ...
        this.axios.post('/api-admin/save-page',qs.stringify({
          ...
          if(res.data.errorCode == 0) {
            checkSuccess('保存成功');
            setTimeout(this.$router.push('membercenter'),3000);
          }else {
            checkFalse('操作失败');
            return false;
          }
    ...
    changeJumpType() {
      ...
      }else if(this.ruleForm.jumpType == 2) {
    ...
    cloneFunctionComponent() {
      ...
      if(this.isRelevance) {
        this.mainList.push(this.merchant);
      }else {
        let temp = {
          key: 'B4',
          elementIndex: 1
    ...
    getDetail(pageId) {
      this.axios.get('/api-admin/page-detail',{
        ...
        if(res.data.errorCode == 0) {
          ...
          this.componentList = tempDetail.value.filter(item => {
            return item.key !== 'B3'
          });
    ...
    getUserInfo() {
      this.axios.get('/api-admin/get-card-setting',{
        params: {
          requestProject: 'gic-web'
        }
      }).then(res => {
        if(res.data.errorCode === 0) {
          ...
          this.curCard = this.userInfo.cardStyle;
        }
      }).catch(err => {
        checkStatus(err);
      })
    ...
    isClique() {
      this.axios.get('/api-auth/is-relevance-clique',{
        ...
        if(res.data.errorCode === 0) {
          this.isRelevance = res.data.result;
          if(res.data.result) {
            this.getList();
          }else {
            this.getIntegral();
