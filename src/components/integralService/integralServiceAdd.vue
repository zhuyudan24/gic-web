              ...
              <div class="container-content">
                <div class="frontcard">
                  <div class="integral-list-page">
                    <div class="integral-list-top" :style="{'height': !ruleForm2.sign.status ? '130px' : ''}">
                      <img class="integral-list-img" v-if="ruleForm2.bgImg.url" :src="ruleForm2.bgImg.url">
                      <img class="integral-list-img" v-else src="../../../static/img/credit-banner_bg.png" alt="">
                      <div class="integral-list-item" v-if="ruleForm2.integral.show">
                        <p class="integral-num"  :style="{color: ruleForm2.integral.numColor}">10000</p>
                        <p class="integral-item-title" :style="{color:ruleForm2.integral.fontColor}" v-if="ruleForm2.integral.title !== ''">{{ ruleForm2.integral.title }}</p>
                        <p class="integral-item-title" :style="{color:ruleForm2.integral.fontColor}" v-else>可用积分</p>
                      </div>
                      <div class="integral-list-item" v-if="ruleForm2.unIntegral.show">
                        <p class="integral-num"  :style="{color: ruleForm2.unIntegral.numColor}">300</p>
                        <p class="integral-item-title" :style="{color:ruleForm2.unIntegral.fontColor}" v-if="ruleForm2.unIntegral.title !== ''">{{ ruleForm2.unIntegral.title }}</p>
                        <p class="integral-item-title" :style="{color:ruleForm2.unIntegral.fontColor}" v-else>冻结积分</p>
                      </div>
                    </div>
                    <div class="integral-list-bottom" style="overflow: hidden;" v-if="ruleForm2.sign.status">
                      <img width="353" height="85" v-if="ruleForm2.sign.before.url" :src="ruleForm2.sign.before.url" alt="">
                ...
                <div class="container-content" style="margin-top: 10px">
                  <draggable :options="{group:{name:'people', pull:false, put:true }}"
                             v-model="componentList" style="min-height: 800px;">
                    <div class="displayItem-new" @click="handlerComponent(item.key,index)" :key="index" v-if="componentList.length > 0" v-for="(item,index) in componentList"
                         :class='["item" + item.key,{"curedit": curIndex == index, "displayItem":(item.key=="B7" || item.key=="A1" || item.key=="A2" || item.key=="A3" || item.key=="A5" || item.key=="A6" || item.key=="A7" || item.key=="A8")}]'>
                      ...
                      <!-- 三张静态图 -->
                      <div v-if="item.key == 'B8'"  class="static-img">
                        <img :src="item.img" alt="">
                      </div>
                      <div v-if="item.key == 'B9'"  class="static-img">
                        <img :src="item.img" alt="">
                      </div>
                      <div v-if="item.key == 'B10'"  class="static-img">
                        <img :src="item.img" alt="">
                      </div>
                      <!-- 卡券 -->
                    ...
                    <div v-if="componentList.length == 3" style="border: 1px dashed #dcdfe6;line-height: 36px;">拖拽添加组件</div>
                  </draggable>
                </div>
              </div>
            </div>
              ...
              <div v-if="curIndex == index && item.key == 'B2'" v-for="(item, index) in componentList" :key="index">
                ...
                <div v-if="curIndex == index" v-for="(item, index) in componentList" :key="index">
                  ...
                  <div v-if="item.templateType !== 3" class="imgwrap" v-for="(list, index) in item.data" :key="index">
                    ...
                    <div class="imgflex2 imgmtop">
                      <span class="imgflex-item1">标签</span>
                      <span  class="formtext">
                         <div class="forminputWidth">
...
<script>
    module.exports = require('./integralServiceAdd')
</script>

<style lang="less" scoped>
@import "./integralServiceAdd";
.integral-list-page {
  padding-bottom: 35px;
  position: relative;
  cursor: pointer;
...
.integral-list-img{
  position: absolute;left: 0;top: 0;bottom: 0;right: 0;width: 100%;z-index: 0
}
.integral-list-item{
  flex:1;
  z-index: 2;
