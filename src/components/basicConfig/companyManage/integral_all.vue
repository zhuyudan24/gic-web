      ...
      <div class="right-box">
        <div class="box-top-wrap clearfix">
          ...
          <div class="box-right fr">
            <el-input @keyup.native.enter="getData(currentPage,pageSize)" clearable class="search-input" prefix-icon="el-icon-search" v-model="searchValue" placeholder="输入积分结算归属名称"></el-input>
...
<script>
  ...
  export default {
    ...
    data(){
        ...
        // 导出数据
        dialogVisible:false,
        excelUrl:'/api-report/integral-summary-data-excel',
        params:{},
        contactDialog:false,
    	}
    },
    ...
    methods:{
      ...
      getData(page,size){   // 获得数据
        this.loading = true;
        this.axios.post('/api-report/integral-summary-data',qs.stringify({
          requestProject:'gic-web',
          month:this.month,
          ...
          if(data.errorCode==0){
            ...
            if(data.result!=null){
              ...
              this.number1 = data.result.totalCountValue.generatIntegralSum;
              this.number2 = data.result.totalCountValue.settlementIntegralSum;
              this.number3 = data.result.totalCountValue.integralSettlementCostSum;
            }
          }else{
            this.$message({
              type: 'error',
      ...
      downloadExcel(){  // 导出数据
        ...
        // this.axios.post('/api-report/integral-summary-data-excel',qs.stringify({
        //   requestProject:this.projectName,
        //   month:this.month,
        //   searchParams:this.searchValue
        // })).then(res=>{
        //   var data = res.data;
        //   // console.log(res);
        //   window.location.href = excelConfig.config + '/api-report/integral-summary-data-excel?requestProject=gic-web&month='+this.month+'&searchParams='+this.searchValue         
        // })
      },
      sensitiveReport(){
