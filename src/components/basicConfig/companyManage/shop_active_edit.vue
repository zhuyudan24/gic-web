	             ...
	             <span class="text-num">{{textNum}}/{{textLength}}</span>
					...
					<div class="input-wrap">
					    ...
					    <!-- <el-switch v-model="open"></el-switch> -->
							...
							<el-radio v-model="radioPeople" label="1">所有会员</el-radio>
	  					<el-radio v-model="radioPeople" label="2">自定义人群</el-radio>
  					</el-form-item>
					</div>
    		</el-form>

    		<div v-if="radioPeople==2" class="select-people">
          ...
          if (valid) {
            if(that.startDate==""){
		        ...
		        if(!that.errorBool2 && !that.errorBool1){
			        ...
			        }else if(that.radioPeople=='1'){	// 自定义
			        	that.searchParams = "{}";
			        	that.sendObj = "{}";
			        	that.saveData()
			        }
