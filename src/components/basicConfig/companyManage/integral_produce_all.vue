      ...
      <div class="right-box">
        ...
        <div class="table-wrap">
          ...
          <el-table
            ...
            <el-table-column
              prop="integralGeneralAmount"
              label="累计产生积分总额">
            </el-table-column>
            <el-table-column
              prop="integralSurplusAmount"
              label="累计积分剩余总额">
            </el-table-column>
            <el-table-column
              prop="integralAmount"
  ...
  export default {
    ...
    data(){
      return {
        ...
        // 导出数据
        dialogVisible:false,
        excelUrl:'/api-report/integral-produce-summary-data-excel',
        params:{},
        contactDialog:false,
      }
    },
    ...
    methods:{
      ...
      getData(page,size){   // 获得数据
        this.loading = true;
        this.axios.post('/api-report/integral-produce-summary-data',qs.stringify({
          requestProject:'gic-web',
          month:this.month,
          ...
          if(data.errorCode==0){
            ...
            if(data.result!=null){
              ...
              this.number1 = data.result.totalCountValue.integralGeneralAmountSum;
              this.number2 = data.result.totalCountValue.integralAmountSum;
              this.number3 = data.result.totalCountValue.integralSurplusAmountSum;
            }
          }else{
            this.$message({
              type: 'error',
      ...
      downloadExcel(){  // 导出数据
        ...
        // this.axios.post('/api-report/integral-produce-summary-data-excel',qs.stringify({
        //   requestProject:'gic-web',
        //   month:this.month,
        //   searchParams:this.searchValue
        // })).then(res=>{
        //   var data = res.data;
        //   console.log(res);
        //   window.location.href = excelConfig.config + '/api-report/integral-produce-summary-data-excel?requestProject=gic-web&month='+this.month+'&searchParams='+this.searchValue        
        // })
      }
    },
    components: {
