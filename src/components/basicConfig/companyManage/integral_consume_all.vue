      ...
      <div class="right-box">
        <div class="box-top-wrap clearfix">
          ...
          <div class="box-right fr">
            <el-input @keyup.native.enter="getData(currentPage,pageSize)" clearable class="search-input" prefix-icon="el-icon-search" v-model="searchValue" placeholder="输入成本主体"></el-input>
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <p>{{scope.row.storeGroupName}}</p>
              </template>
            </el-table-column>
            <el-table-column
              prop="integralUseValue"
              label="月消耗总额">
            </el-table-column>
            <template slot="empty">
  ...
  export default {
    ...
    data(){
      return {
        ...
        // 导出数据
        dialogVisible:false,
        excelUrl:'/api-report/integral-consume-summary-data-excel',
        params:{},
        contactDialog:false,
      }
    },
    ...
    methods:{
      ...
      getData(page,size){   // 获得数据
        this.loading = true;
        this.axios.post('/api-report/integral-consume-summary-data',qs.stringify({
          requestProject:'gic-web',
          month:this.month,
          ...
          if(data.errorCode==0){
            ...
            if(data.result!=null){
              ...
              this.number1 = data.result.totalCountValue.integralUseValueSum;
            }
          }else{
            this.$message({
              type: 'error',
      ...
      downloadExcel(){  // 导出数据
        ...
        // this.axios.post('/api-report/integral-consume-summary-data-excel',qs.stringify({
        //   requestProject:'gic-web',
        //   month:this.month,
        //   searchParams:this.searchValue
        // })).then(res=>{
        //   var data = res.data;
        //   console.log(res);
        //   window.location.href = excelConfig.config + '/api-report/integral-consume-summary-data-excel?requestProject=gic-web&month='+this.month+'&searchParams='+this.searchValue          
        // })
      }
    },
    components: {
