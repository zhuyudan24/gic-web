import customAddLeft from './customAddLeft.vue'
...
const mainListData = bases.mainList;
export default {
  mixins: [minxinbase],
  data() {
    return {
  ...
  methods: {
    ...
    submitForm(saveType) {
      ...
      if(this.extitle !== this.ruleForm.title) {
        this.axios.post('/api-admin/is-exist-page-name',qs.stringify({
          ...
          }else if(res.data.errorCode=='110019'){
            this.$message.warning('页面名称不能重复')
          }else{
    ...
    async comitForm(saveType) {
        ...
        if(item.key == 'B7'){
          ...
          item.data.forEach((list,idx) => {
            if(list.status) {
              ...
              // if(list.visibleCondition==1 && !list.limitCondition) {
              //   checkFalse(`请填写第${ index + 1 }个组件(数据icon)中的可见人群的限定条件`);
              //   this.flag = false;
              // }
            }
          });
      ...
      let params = {
        pageId: this.pageId,
        title: this.ruleForm.title,
        entryCondition: this.ruleForm.entryCondition,
        pageType: 1,
        value: value
      };
      // console.log(params);
      ...
      if(this.saveBtnBool){
        ...
        this.axios.post('/api-admin/save-page',qs.stringify({
          ...
          if(res.data.errorCode == 0) {
            checkSuccess('保存成功');
            setTimeout(this.$router.push('custom'),3000);
          }else {
            checkFalse('操作失败');
            return false;
          }
    ...
    cloneFunctionComponent() {
      this.mainList = _.cloneDeep(mainListData);
      this.mainList.push(this.integraldata);
    },
    // 获取基础组件详情
    getDetail(pageId) {
    ...
    // 获取功能组件详情
    getIntegral() {
      this.axios.get('/api-admin/get-integral-setting',{
        params: {
          requestProject: 'gic-web'
        }
      }).then(res => {
