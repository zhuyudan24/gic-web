...
export default {
  ...
  data() {
    return {
      ...
      // 导出数据
      dialogVisible:false,
      excelUrl:'/api-admin/report-send-commission-settle-refund-detail',
      params:{}
    }  
  },
  mounted(){
  ...
  methods: {
    ...
    getList(){
      this.axios.post('/api-admin/list-send-commission-settle-refund-detail',qs.stringify({
        requestProject:this.projectName,
    ...
    operation(h,label){
      ...
      if(label=="结算时间"){
        ...
        arr = [
          h('span',{},[
            <span slot="content">该发货单的发货门店信息</span>
          ])
        ]
      }else if(label=="发货导购"){
        arr = [
