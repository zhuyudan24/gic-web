...
export default {
  ...
  data() {
    return {
      ...
      // 导出数据
      dialogVisible:false,
      excelUrl:'/api-admin/report-clerk-commission-collect',
      params:{},
      // 选择门店
      uuid:'',  // 新增传空，编辑必传,
  ...
  methods: {
    ...
    getList(){
      this.loading = true;
      this.axios.post('/api-admin/list-clerk-commission-collect',qs.stringify({
        requestProject:this.projectName,
    ...
    operation(h,label){
      ...
      if(label=="归属佣金"){
        ...
        arr = [
          h('span',{},[
            <span slot="content">发货佣金所关联的货品，发生了退款维权事件，会对原有的佣金进行追扣。</span>
          ])
        ]
      }else if(label=="总佣金"){
        arr = [
